...
{
    public static class ValidadorImagens 
    {


        private bool Extensao(string teste)
            {
        ...
        public static async Task<ValidadorArquivo> UploadArquivo(IFormFile arquivo, string imgNome)
        {
            ...
            {
				return new ValidadorArquivo(){ Status = false, Mensagem = "Erro ao subir arquivo" };
            }
			...
			if (Extensao(extensaoArquivo))
				return new ValidadorArquivo(){ Status = true, Mensagem = "OK Upload" };

			else return new ValidadorArquivo(){ Status = false, Mensagem = "Erro ao subir arquivo, extensão" };
        }
